...
```{r}
# Graph 6: Choropleth map with clickable countries (key = Country_Region)
renderPlotly({
  plot_ly(covid_summary, type = "choropleth",
          ...
          key = ~Country_Region,
          colorscale = 'Viridis') %>%
    layout(title = 'COVID-19 Cases per 100K by Country')
})
```
...
```{r}
#| context: server
# Reactive selection based on user clicking a country on the map (Graph 6)
...
# Graph 4: Time series of cases for selected country
output$tscases <- renderPlotly({
  req(selected_country())
  covid %>%
    filter(Country_Region == selected_country()) %>%
    plot_ly(x = ~Date, y = ~positive, type = "scatter", mode = "lines+markers",
            name = "Cases")
})

# Graph 5: Time series of deaths for selected country
output$tsdeaths <- renderPlotly({
  req(selected_country())
  covid %>%
    filter(Country_Region == selected_country()) %>%
    plot_ly(x = ~Date, y = ~death, type = "scatter", mode = "lines+markers",
            name = "Deaths")
})
